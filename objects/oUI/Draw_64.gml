...
draw_text(10, 655, "Comrade : ");
len = oPatient.hp;
for (i =0; i < len; i += 1) {
	draw_sprite(sHealth,1,100 + (len * (i + 1)) + (i * sprite_get_width(sHealth)),655);
}
...
len = oPlayer.hp;
for (i =0; i < len; i += 1) {
	draw_sprite(sHealth,0,10 + (len * (i + 1)) + (i * sprite_get_width(sHealth)),685);
}
